 ...
 * ## Qué protege aquí
 *
 * El login del panel. `/api/auth/*` queda **fuera** del matcher de `proxy.ts`,
 * así que se puede martillear el endpoint de credenciales directamente, sin
 * pasar por la pantalla. Con contraseñas humanas de una tienda pequeña, eso es
 * el objetivo de un ataque de diccionario.
 *
 * ## Copia deliberada
 *
 * Este archivo es gemelo de `storefront/lib/rate-limit.ts`. No se comparte
 * porque **son dos despliegues distintos**: la tienda va a mudarse a su propio
 * repositorio y no puede depender de un módulo del panel. Cuarenta líneas
 * duplicadas cuestan menos que un acoplamiento que hay que deshacer.
 *
