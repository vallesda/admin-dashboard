  ...
  return (
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden rounded-sm bg-sand">
        ...
        {/* Availability is a word, never only a dimmed image: colour alone
            would not survive greyscale or bright sunlight. */}
        {!product.availableForSale ? (
          <span className="absolute left-2 top-2 rounded-sm bg-foreground/85 px-2 py-1 text-xs text-background">
            Agotado
          </span>
        ) : product.seasonal ? (
          <span className="absolute left-2 top-2 rounded-sm bg-gold px-2 py-1 text-xs font-medium text-foreground">
            De temporada
          </span>
        ) : null}
      </div>
